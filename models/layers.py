...
from tensorflow.keras.layers import LSTMCell, Dense, Activation, BatchNormalization
...
class ContrastiveNoiseLayer(tf.keras.layers.Layer):
    def __init__(self, duplicates, **kwargs):
        super(ContrastiveNoiseLayer, self).__init__(**kwargs)
        self.contrastive_duplicates = duplicates
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        output_shape = tf.shape(tf.stack([tf.zeros_like(inputs)
                                          for _ in range(self.contrastive_duplicates)], axis=-2))

        contrastive_samples = tf.random.uniform(shape=output_shape, minval=min_val, maxval=max_val, dtype=tf.float64)
...
class ResBlock(tf.keras.layers.Layer):
    def __init__(self, hidden_size, **kwargs):
        ...
        self.dense0 = Dense(hidden_size, activation=None, kernel_initializer='he_uniform')
        # self.batch_norm0 = BatchNormalization()
        self.activation0 = Activation('tanh')
        self.dense1 = Dense(1, activation=None, kernel_initializer='he_uniform')
        self.dense2 = Dense(1, activation=None, kernel_initializer='he_uniform')
        # self.batch_norm1 = BatchNormalization()
        # self.dense1 = Dense(hidden_size, activation=None, kernel_initializer='he_uniform')
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        return out + self.dense2(inputs)

class NoiseLayer(tf.keras.layers.Layer):
    def __init__(self, noise_std, name="noise"):
        super(NoiseLayer, self).__init__(name=name)
...
class LSTMCellContrastive(LSTMCell):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(LSTMCellContrastive, self).__init__(units, **kwargs)
        self.units = units
    ...
    def call(self, inputs_stacked, states, training=None):
        ...
        h = K.concatenate([h_1] + h_contrastive, axis=-1)
        recurrent_states = [h_1, c_1]
        return h, recurrent_states


class LSTMContrastive(RNN):
    ...
    def __init__(self, units,
                 ...
                 contrastive_duplicates=1,
                 **kwargs):
        cell = LSTMCellContrastive(units,
        ...
        super(LSTMContrastive, self).__init__(cell,
